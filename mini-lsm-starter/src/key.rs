...
impl<T: AsRef<[u8]>> Key<T> {
    ...
    pub fn encode_len(&self) -> usize {
        2 * std::mem::size_of::<u16>() + self.raw_len()
    }

    pub fn encode(&self, buf: &mut Vec<u8>) {
        // encode format:
        // |raw key len|key len|key|ts|
        buf.extend_from_slice(&(self.raw_len() as u16).to_be_bytes());
        buf.extend_from_slice(&(self.key_len() as u16).to_be_bytes());
        buf.extend_from_slice(self.0.as_ref());
        buf.extend_from_slice(self.1.to_be_bytes().as_ref());
    }
...
impl Key<Bytes> {
    ...
    pub fn decode(mut buf: impl Buf) -> Self {
        let _raw_len = buf.get_u16() as usize;
